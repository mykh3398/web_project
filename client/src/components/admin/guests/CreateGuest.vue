<template>
        ...
        <form class="needs-validation m-3" name="formSignUp" id="idFormSignUp" 
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="idLastName" name="lastName" pattern="^[A-ZА-ЯЄЇІ][A-Za-zА-Яа-яЄєЇїІі\s]+$" required
                    v-model="guest.Last_Name" @change="checkValidation" @input="(event) => resetClassList(event)">
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="idFirstName" name="firstName" pattern="^[A-ZА-ЯЄЇІ][A-Za-zА-Яа-яЄєЇїІі\s]*$" required
                    v-model="guest.First_Name" @change="checkValidation" @input="(event) => resetClassList(event)">
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="idMiddleName" name="middleName" pattern="^[A-ZА-ЯЄЇІ][A-Za-zА-Яа-яЄєЇїІі\s]*$" 
                    v-model="guest.Middle_Name" @change="checkValidation" @input="(event) => resetClassList(event)">
                <div class="invalid-feedback">
            ...
            <div class="form-check">
                <input class="form-check-input" type="radio" name="flexRadio" id="flexRadioMan" v-model="guest.Gender" value="1" required>
                <label class="form-check-label" for="flexRadioMan">
                    Man
                </label>
            </div>
            <div class="form-check">
                <input class="form-check-input" type="radio" name="flexRadio" id="flexRadioWoman" v-model="guest.Gender" value="2" required>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control item" id="idBirthday" name="birthday" pattern="\d{2}.\d{2}.\d{4}" required 
                    :max="maxDate"
                    v-model="guest.Passport_Date" @change="checkValidation" @input="(event) => resetClassList(event)" />
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="idPhone" name="phone" required
                  placeholder="+38(0__) ___-__-__" pattern="^\+38\(0\d{2}\)\s\d{3}[\-]\d{2}[\-]\d{2}$" 
                  v-model="guest.Phone" @change="checkValidation" @input="(event) => resetClassList(event)">
              <div class="invalid-feedback">
            ...
            <!-- <div class="mb-3">
                <label for="idUser" class="form-label">Middle Name</label>
                <input type="text" class="form-control" id="idUser" name="user" pattern="^[A-ZА-ЯЄЇІ][A-Za-zА-Яа-яЄєЇїІі\s]*$" 
                    v-model="guest.ID_Ref_User" @change="checkValidation" @input="(event) => resetClassList(event)">
                <div class="invalid-feedback">
                    Please enter existing user
                </div>            
            </div> -->
           
            <button type="button" name="signUp" class="btn btn-primary" @click="createGuest" disabled>
                Create Guest
            </button>&nbsp;
            <button type="reset" name="cancel" class="btn btn-secondary" @click="(event) => onCancel(event)" disabled>
                Cancel
            </button>&nbsp;    
  ...
  export default {
    ...
    methods: {
      async createGuest() {
        try {
          const response = await axios.post(this.apiUrl + this.guestsEndpoint, this.guest);
          if (response.status === 200) {
            this.guest = { ID_Guest: '', First_Name: '', Middle_Name: '', Last_Name: '', 
            Passport_Number: '', Phone: '', Passport_Date: '', Gender: '', ID_Ref_User: null};
            await this.$router.push('/guests')
          } else {
